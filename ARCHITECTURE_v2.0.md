...
PyMD 是一个**本地优先**的文档工作区，第一阶段专注于 Markdown 编辑与多格式转换（PDF ↔ Markdown ↔ Word/PPT/Excel）。支持本地单机运行或个人服务器 File/DataHub 模式。

> 以下架构图和模块设计反映了三栏布局、AI 集成及知识引擎的长期愿景，部分内容超出第一阶段范围，供参考。
